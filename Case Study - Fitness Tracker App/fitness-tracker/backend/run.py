...
def create_app():
    ...
    def get_filtered_workouts():
        user_id = int(get_jwt_identity())
        range_param = request.args.get("range", "5days")  # default last 5 days

        today = datetime.today().date()
        if range_param == "3days":
            start_date = today - timedelta(days=2)  # last 3 days including today
        elif range_param == "5days":
            start_date = today - timedelta(days=4)  # last 5 days including today
        elif range_param == "month":
            start_date = today.replace(day=1)  # first day of current month
        else:
            start_date = today - timedelta(days=4)  # fallback to last 5 days

        workouts = Workout.query.filter(
            Workout.user_id == user_id,
            Workout.date >= start_date,
            Workout.date <= today
        ).order_by(Workout.date.desc()).all()  # newest first
    ...
    def get_user_stats():
        ...
        # Calculate total workouts and total calories
        workouts = Workout.query.filter_by(user_id=user_id).all()
    ...
    # -------- Progress: Filtered Analytics --------
    @app.route("/progress/analytics", methods=["POST"])
    @jwt_required()
    def filtered_analytics():
        ...
        # Average duration
        avg_duration = (
            db.session.query(func.avg(Workout.duration))
            .filter(Workout.user_id == user_id)
            .filter(Workout.date >= datetime.strptime(start_date, "%Y-%m-%d").date() if start_date else True)
            .filter(Workout.date <= datetime.strptime(end_date, "%Y-%m-%d").date() if end_date else True)
            .scalar() or 0
        )

        # Calories per exercise type
        calories_per_type = (
            db.session.query(Workout.exercise_type, func.sum(Workout.calories))
            .filter(Workout.user_id == user_id)
            .filter(Workout.date >= datetime.strptime(start_date, "%Y-%m-%d").date() if start_date else True)
        ...
        calories_data = [{"exercise_type": c[0], "calories": c[1]} for c in calories_per_type]

        return jsonify({
            "total_calories": total_calories,
            "avg_duration": float(avg_duration),
            "calories_per_type": calories_data,
            "total_workouts": len(workouts)
        }), 200
